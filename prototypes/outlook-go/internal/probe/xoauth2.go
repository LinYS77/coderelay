...
// xoauth2Client implements the Outlook XOAUTH2 SASL initial response:
// user=<email>\x01auth=Bearer <access-token>\x01\x01.
// It retains the response only until Authenticate returns so it can be cleared.
...
func (c *xoauth2Client) Next(_ []byte) ([]byte, error) {
	// Outlook sends a base64-encoded JSON challenge on authentication failure.
	// The protocol requires an empty response before the tagged failure. Never
	// parse or surface the challenge because it may contain sensitive details.
	return []byte{}, nil
}
